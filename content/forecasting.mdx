...
### 🔮 GPU-Accelerated Forecasting System  
...
- **Frontend (React + Next.js)** — CSV upload, configuration, and forecast visualization  
- **Backend (Python)** — Model training, inference, probabilistic intervals  
- **AI Engine (NHITS)** — GPU-accelerated forecasting using PyTorch  
- **Database (MongoDB)** — ProjectForecasting & metadata storage  
- **API Layer (FastAPI)** — `/forecast/` endpoint for real-time predictions  

<br />



### 🖥️ Frontend — Forecast Creation Form
...
This corresponds to NHITS’ horizon parameter.

<br />

#### **Confidence Level (%)**  
Optional numeric field (80–100).  
Determines the width of probabilistic forecast intervals.  
- Higher confidence → wider bands  
- Lower confidence → narrower bands  

Values map to NHITS’ `level` parameter.
...
### 🧠 NHITS Forecasting Engine (PyTorch + NeuralForecast)
