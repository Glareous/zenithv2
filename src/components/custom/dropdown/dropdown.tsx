...
let openDropdowns: React.RefObject<HTMLDivElement | null>[] = []
    ...
    if (
      ...
      if (openDropdowns.length > 2) {
        openDropdowns = openDropdowns.filter(
          (item: React.RefObject<HTMLDivElement | null>) => {
            count++
            if (count === 2) {
              if (item.current) {
                item.current.click()
...
const DropdownButton: React.FC<DropdownButtonProps> = ({
  ...
  return (
    <button className={`${colorClass}`} type="button">
      ...
      {arrow && (
        <svg
          className={`size-5 arrow group-data-[sidebar=small]:hidden ${isOpen ? 'transform rotate-180' : ''}`}
          viewBox="0 0 20 20"
          fill="currentColor">
          <path
            fillRule="evenodd"
