...
const singleChoiceSchema = z.object({
  ...
  choices: z
    .array(
      z.object({
        value: z.string().min(1, 'Choice cannot be empty'),
      })
    )
    .min(2, 'At least 2 choices are required'),
})

type SingleChoiceFormData = z.infer<typeof singleChoiceSchema>

interface DrawerSingleChoiceProps {
  isOpen: boolean
  onClose: () => void
  onSave: (data: SingleChoiceFormData) => void
  onDelete?: () => void
  initialData?: {
    identifier?: string
    prompt?: string
...
const DrawerSingleChoice: React.FC<DrawerSingleChoiceProps> = ({
  isOpen,
  onClose,
  onSave,
  onDelete,
  ...
  const [isChoicesAccordionOpen, setIsChoicesAccordionOpen] = useState(true)
  ...
  } = useForm<SingleChoiceFormData>({
    resolver: zodResolver(singleChoiceSchema),
    mode: 'all',
    defaultValues: {
      identifier: '',
      question: '',
  ...
  const onSubmit = (data: SingleChoiceFormData) => {
    onSave(data)
    reset()
    onClose()
  }
  ...
  const handleEditMode = () => {
    ...
    setTimeout(() => {
      setIsChoicesAccordionOpen(false)
      setTimeout(() => setIsChoicesAccordionOpen(true), 10)
    }, 10)
  }

  React.useEffect(() => {
    if (isOpen) {
      const defaultChoices = initialData?.choices?.map((choice) => ({
        value: choice,
      })) || [{ value: '' }, { value: '' }]
  ...
  const content = (
    <form
      ...
      <div className="mt-2">
        <Accordion
          title="Choices"
          isOpen={isChoicesAccordionOpen}
          onToggle={() => setIsChoicesAccordionOpen(!isChoicesAccordionOpen)}
...
export default DrawerSingleChoice
