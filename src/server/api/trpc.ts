...
export const protectedProcedure = t.procedure
  ...
  .use(({ ctx, next }) => {
    ...
    return next({
      ctx: {
        // infers the `session` as non-nullable or provides apiKeyUser
        session: ctx.session ? { ...ctx.session, user: ctx.session.user } : { user },
 ...
 * 
 * This procedure only accepts API key authentication and is designed for REST endpoints
 */
