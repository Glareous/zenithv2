...
export const projectRouter = createTRPCRouter({
  ...
  delete: protectedProcedure
    ...
    .mutation(async ({ ctx, input }) => {
      ...
      */
      ...
      // 4. Clear pqrAgentId if set (because of NoAction constraint)
      if (project.pqrAgentId) {
        console.log('🔗 PROJECT_DELETE_CLEARING_PQR_AGENT:', {
          projectId: input.id,
          pqrAgentId: project.pqrAgentId,
        })
        await ctx.db.project.update({
          where: { id: input.id },
          data: { pqrAgentId: null },
        })
      }

      // 5. Delete the project and all related data in a transaction
      try {
  ...
  // Update PQR agent for a project
  updatePQRAgent: protectedProcedure
    .input(
      z.object({
        projectId: z.string(),
        agentId: z.string(),
    ...
    .mutation(async ({ ctx, input }) => {
      ...
      // Verify the agent exists and belongs to this project
      ...
      // Update the project with the PQR agent
      const updatedProject = await ctx.db.project.update({
        where: { id: input.projectId },
        data: {
          pqrAgentId: input.agentId,
        },
        include: {
          pqrAgent: true,
        },
  ...
  // Remove PQR agent from a project
  removePQRAgent: protectedProcedure
    .input(
      z.object({
        projectId: z.string(),
      })
    ...
    .mutation(async ({ ctx, input }) => {
      ...
      // Update the project to remove PQR agent
      const updatedProject = await ctx.db.project.update({
        where: { id: input.projectId },
        data: {
          pqrAgentId: null,
