...
export const projectProductFileRouter = createTRPCRouter({
  // Get presigned URL for uploading product images
  getUploadUrl: protectedProcedure
    ...
    .mutation(async ({ ctx, input }) => {
      console.log('🔍 getUploadUrl called for product file:', {
      ...
      const s3Key = `products/${input.productId}/files/${uniqueFileName}`
  ...
  create: protectedProcedure
    ...
    .mutation(async ({ ctx, input }) => {
      console.log('💾 create product file called with:', {
        productId: input.productId,
        fileName: input.fileName,
        fileType: input.fileType,
      ...
      console.log('💾 Creating product file record with:', {
        s3Url,
        bucketName: env.S3_BUCKET_NAME,
        userId: ctx.session.user.id,
      })

      const file = await ctx.db.projectProductFile.create({
        data: {
          name: input.name,
          fileName: input.fileName,
          fileType: input.fileType,
  ...
  getAll: protectedProcedure
    ...
    .query(async ({ ctx, input }) => {
      ...
      const files = await ctx.db.projectProductFile.findMany({
        where: {
          productId: input.productId,
      ...
      const totalCount = await ctx.db.projectProductFile.count({
        where: {
          productId: input.productId,
          ...(input.fileType && { fileType: input.fileType }),
        },
  ...
  getById: protectedProcedure
    ...
    .query(async ({ ctx, input }) => {
      const file = await ctx.db.projectProductFile.findFirst({
        where: {
          id: input.id,
          product: {
            project: {
  ...
  delete: protectedProcedure
    ...
    .mutation(async ({ ctx, input }) => {
      ...
      // Delete from DB
      await ctx.db.projectProductFile.delete({
        where: { id: input.id },
      })
