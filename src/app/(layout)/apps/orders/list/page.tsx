...
import OrderStatsCards from '@src/app/(layout)/apps/ecommerce/orders/list/OrderStatsCards'
  ...
  const columns = useMemo(
    () => [
      ...
      },
      {
        ...
        cell: ({ row }: { row: { original: OrderData } }) => (
          <Link
            href={`/apps/orders/overview?id=${row.original.id}`}
      ...
      },
      {
        ...
        accessorKey: 'type',
        cell: ({ row }: { row: { original: OrderData } }) => (
          <span className={getTypeClass(row.original.type)}>
            {row.original.type}
          </span>
        ),
      },
      ...
      },
      {
        ...
        cell: ({ row }: { row: { original: OrderData } }) => {
          ...
          const itemType =
            order.type === 'PRODUCT' ? 'product(s)' : 'service(s)'
          return (
            <span>
              {itemCount} {itemType}
            </span>
          ...
          <Dropdown
            ...
            <DropdownMenu>
              <Link
                ...
                onClick={(e) => {
                  e.preventDefault()
                  router.push(`/apps/orders/overview?id=${row.original.id}`)
                }}>
              ...
              <Link
                ...
                onClick={(e) => {
                  e.preventDefault()
                  router.push(`/apps/orders/create-order?id=${row.original.id}`)
                }}>
        ...
        <div className="grid items-center grid-cols-12 gap-3 xl:gap-5 card-header">
          <div className="flex flex-wrap xl:flex-nowrap justify-end col-span-12 gap-3 xl:col-span-12 2xl:col-span-5">
            ...
            <button
              ...
              onClick={() => router.push('/apps/orders/create-order')}>
