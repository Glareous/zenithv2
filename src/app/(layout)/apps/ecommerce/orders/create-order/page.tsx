...
const CreateOrder: NextPageWithLayout = () => {
  ...
  }, [isEditMode, orderId, refetchOrderData])

  const createOrder = api.projectOrders.create.useMutation({
    onMutate: () => {
      console.log('🚀 createOrder.mutate iniciando...')
    },
    onSuccess: async (newOrder) => {
      console.log('✅ createOrder.mutate SUCCESS - INICIANDO')
      console.log('📦 newOrder recibido:', newOrder)

      if (!newOrder) {
        console.log('❌ newOrder es null/undefined')
        return
      }
      ...
      router.push('/apps/ecommerce/orders/list')

      console.log('✅ createOrder.mutate SUCCESS - COMPLETADO')
    },
    onError: (error) => {
      console.error('❌ createOrder.mutate ERROR:', error)
  ...
  const getAvailableStock = (productId: string, warehouseId: string) => {
    ...
    if (isEditMode) {
      console.log('Edit mode - returning full warehouse stock')
