...
import { Filter, Grid, History, Plus, Search, Trash } from 'lucide-react'
...
const List: NextPageWithLayout = () => {
  ...
  const handleChangeStatusProduct = useCallback((product: ProductData) => {
  ...
  const handleViewTransactions = useCallback(
    (product: ProductData) => {
      router.push(`/apps/ecommerce/products/transactions/${product.id}`)
    },
    [router]
  )
  ...
  const handleSelectAll = useCallback(() => {
    ...
    if (selectAll) {
      ...
      setDeletedListData(products.map((product) => product.id))
    }
    setSelectAll((prev) => !prev)
  }, [selectAll, productsData])
  ...
  const getStockDisplay = (warehouses: ProductData['warehouses']) => {
    if (!warehouses || warehouses.length === 0) return '0'
    return warehouses.map((w) => w.stock).join(', ')
  }
  ...
  const getWarehousesDisplay = (warehouses: ProductData['warehouses']) => {
    if (!warehouses || warehouses.length === 0) return 'No warehouse'
    return warehouses.map((w) => w.warehouse.warehouseId).join(', ')
  }
  ...
  const getTotalQuantity = (warehouses: ProductData['warehouses']) => {
    if (!warehouses || warehouses.length === 0) return 0
    return warehouses.reduce((sum, w) => sum + (w.stock || 0), 0)
  }

  const getWarehousesWithStock = (warehouses: ProductData['warehouses']) => {
    if (!warehouses || warehouses.length === 0) return 'No warehouse'
    return warehouses
      .map((w) => `${w.warehouse.warehouseId}: ${w.stock}`)
      .join(', ')
  }
  ...
  const columns = useMemo(
    () => [
            ...
            <div className="flex items-center gap-2">
              <div className="flex items-center justify-center p-1 border border-gray-200 rounded-sm dark:border-dark-800 size-9">
                {firstImage ? (
                  ...
                  <div className="w-6 h-6 bg-gray-200 rounded-sm flex items-center justify-center truncate max-w-28">
      ...
      {
        header: () => 'Warehouse',
        accessorKey: 'warehouse_names',
        cell: ({ row }: { row: { original: ProductData } }) => (
          <div className="flex flex-col gap-1">
            {row.original.warehouses.map((w, index) => (
              <span
                key={w.warehouse.id}
                className={`text-xs font-mono px-2 py-1 rounded-md ${
                  w.warehouse.isDefault
                    ? 'bg-blue-100 text-blue-800 dark:bg-blue-900/20 dark:text-blue-300'
                    : 'bg-gray-100 text-gray-700 dark:bg-gray-800 dark:text-gray-300'
                }`}>
                {w.warehouse.warehouseId}
                {w.warehouse.isDefault && (
        ...
        cell: ({ row }: { row: { original: ProductData } }) => (
          <div className="flex flex-col gap-1">
            {row.original.warehouses.map((w, index) => (
              <span
                key={w.warehouse.id}
                className={`text-sm font-medium px-2 py-1 rounded-md text-center ${
                  w.stock === 0
      ...
      },
      {
        ...
        cell: ({ row }: { row: { original: ProductData } }) => {
          const totalQuantity = getTotalQuantity(row.original.warehouses)
      ...
      },
      {
        ...
        accessorKey: 'revenue',
        cell: ({ row }: { row: { original: ProductData } }) => {
          const revenueValue = `$${(row.original.price || 0).toFixed(2)}`
          const displayRevenue =
            revenueValue.length > 6
              ? `${revenueValue.slice(0, 6)}...`
              : revenueValue
          return (
            <Link
              href="#!"
          ...
          <Dropdown
            ...
            <DropdownMenu>
              ...
              <button
                ...
                <i className="align-middle ltr:mr-2 rtl:ml-2 ri-history-line"></i>
