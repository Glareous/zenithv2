  ...
  const columns = useMemo(
    () => [
      ...
      },
      {
        ...
        cell: ({ row }: { row: { original: OrderData } }) => (
          <Link
            href={`/apps/service/orders/overview?id=${row.original.id}`}
      ...
      },
      {
        ...
        cell: ({ row }: { row: { original: OrderData } }) => {
          ...
          if (services.length === 0) {
            return <span className="text-gray-400">No services</span>
          }
          if (services.length === 1) {
            return <span>{services[0].service.name}</span>
          }
          return (
            <span>
            ...
            <DropdownMenu>
              <Link
                ...
                onClick={(e) => {
                  e.preventDefault()
                  router.push(
                    `/apps/service/orders/overview?id=${row.original.id}`
                  )
                }}>
              ...
              <Link
                ...
                onClick={(e) => {
                  e.preventDefault()
                  router.push(
                    `/apps/service/orders/create-orders?id=${row.original.id}`
                  )
                }}>
    ...
    [deletedListData, selectAll, handleSelectAll, router]
  )

  if (loadingOrders) {
    return (
      <React.Fragment>
        <BreadCrumb title="Service Order List" subTitle="Service Orders" />
        <div className="card">
        ...
        <div className="grid items-center grid-cols-12 gap-3 xl:gap-5 card-header">
          <div className="flex flex-wrap xl:flex-nowrap justify-end col-span-12 gap-3 xl:col-span-12 2xl:col-span-5">
            ...
            <button
              ...
              onClick={() => router.push('/apps/service/orders/create-orders')}>
