...
export default function LoginPage() {
  ...
  const onSubmit = async (data: SignInFormData) => {
    try {
      ...
      // Validate login permissions using tRPC (check if user should use slug login)
      try {
  ...
  const handleUserLogin = async () => {
    ...
    try {
      ...
      if (result?.ok) {
        ...
        toast.error('User login failed! Please check the credentials.')
      }
    } catch (error) {
            ...
            <div className="md:p-10 card-body">
              ...
              {/* <p className="mb-5 text-center text-gray-500 dark:text-dark-500">
                ...
                <Link
                  href={
                    invitationToken
                      ? `/auth/signup-basic?invitation=${invitationToken}&email=${invitationEmail}&project=${invitationProject}`
                      : '/auth/signup-basic'
                  }
              ...
              {/* <div className="flex flex-col gap-2">
                <button
                  ...
                  <i className="ri-discord-fill text-[20px] inline-block ltr:mr-1 rtl:ml-1 size-4 text-primary-500"></i>{' '}
